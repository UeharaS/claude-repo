...
### 8. Notionにレーティングログを保存
Notion MCP を使って、ニュースログDB（data_source_id: b94982fa-82c8-4360-9a93-a8215f15f629）に以下のページを作成する。
- タイトル: YYYY-MM-DD 週次レーティング更新（当日の日付）
- 日付: 当日の日付
- 本文: 全銘柄の判定結果（割安判定・PER/PBR/ROE・理由・コメント）をまとめて記載。LINE通知と同等以上の情報量で、ソースURL等も含める
...
### 10. LINE通知②（Notionログリンク）
Step 8 で作成した Notion ページの URL を送信する。
...
curl -s -X POST https://api.line.me/v2/bot/message/push \
  ...
  -d '{
    "to": "'"$LINE_USER_ID"'",
    "messages": [{"type": "text", "text": "📝 週次レーティングログ → {NotionページURL}"}]
  }'
```
